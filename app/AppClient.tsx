...
function ChemicalModule({ recentIds, setRecentIds, navigate }: { recentIds: number[]; setRecentIds: (value: number[]) => void; navigate: (page: PageId) => void }) {
  ...
  const filters = ["全部", "易燃", "有毒", "腐蚀", "氧化", "爆炸", "一般工业品"];
  ...
  const filtered = useMemo(() => {
    ...
    return chemicals.filter((chemical) => {
      ...
      const matchesFilter = filter === "全部" || (filter === "一般工业品" ? chemical.tags.length === 0 : chemical.tags.some((tag) => tag.includes(filter)));
  ...
  return (
    <div className="page">
      <PageHeader eyebrow="CHEMICAL SAFETY" title="化学品安全查询" description="检索 100 种常见化学品理化性质、安全风险、防护与应急处置信息。" actions={<button className="button button-outline" onClick={() => setFilter("全部")}>重置筛选</button>} />
      <section className="search-panel">
        <div className="search-box"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="输入中文名、英文名、CAS 号、分子式或危险关键词" autoFocus /><kbd>{filtered.length} 条</kbd></div>
      ...
      <div className="result-toolbar"><p>找到 <strong>{filtered.length}</strong> 种化学品</p><div><span>绿色 一般</span><span>黄色 中等</span><span>橙色 较高</span><span>红色 重大</span></div></div>
      {filtered.length ? <section className="chemical-grid">
        {filtered.map((chemical) => (
          <article className="chemical-card" key={chemical.id}>
            ...
            <div className="tag-row">{chemical.tags.slice(0, 4).map((tag) => <span key={tag}>{tag}</span>)}</div>
...
function InspectionModule({ history, setHistory, navigate }: { history: InspectionRecord[]; setHistory: (value: InspectionRecord[]) => void; navigate: (page: PageId) => void }) {
  ...
  const selectEquipment = (value: string) => { setEquipment(value); setStep(0); setAnswers({}); };
  const exportReport = () => {
    if (!report) return;
    ...
    <section className="inspection-layout">
      ...
      <div className="inspection-main">
        <section className="panel inspection-meta">
          <div><label>设备名称<input value={equipment} readOnly /></label><label>设备位号<input value={tag} onChange={(event) => setTag(event.target.value)} /></label><label>检查人员<input value={inspector} onChange={(event) => setInspector(event.target.value)} /></label></div>
        ...
        {current ? <section className="panel wizard-card">
          ...
          <div className="inspection-inputs"><label>实测值 / 现场情况<input placeholder="例如：78.5 ℃、0.32 MPa" value={answers[current.id]?.value || ""} onChange={(event) => updateAnswer(current, { value: event.target.value })} /></label><label>现场备注<textarea placeholder="补充设备状态、照片编号或异常描述" value={answers[current.id]?.note || ""} onChange={(event) => updateAnswer(current, { note: event.target.value })} /></label></div>
...
function CleanerModule() {
  ...
  const setValue = (key: keyof CleanerInputs, value: string) => setValues({ ...values, [key]: Number(value) || 0 });
  ...
  return <div className="page">
    <PageHeader eyebrow="CLEANER PRODUCTION" title="清洁生产与能耗排放评价" description="输入生产周期数据，自动计算资源能源指标、综合得分和优先改进方向。" actions={<button className="button button-outline" onClick={() => window.print()}>打印评价报告</button>} />
    <section className="cleaner-layout"><div className="panel cleaner-form"><div className="panel-head"><div><h2>生产过程数据</h2><p>示例单位按周期总量填写</p></div><button className="text-button" onClick={() => setValues({ ...values, water: 0, electricity: 0, steam: 0, gas: 0, wastewater: 0, wastegas: 0, solid: 0 })}>清空消耗数据</button></div><div className="form-section"><h3>产量与原料</h3><div className="input-grid">{inputs.slice(0,4).map(([key,label,unit]) => <NumberField key={key} label={label} unit={unit} value={values[key]} onChange={(value) => setValue(key,value)} />)}</div></div><div className="form-section"><h3>资源与能源</h3><div className="input-grid">{inputs.slice(4,10).map(([key,label,unit]) => <NumberField key={key} label={label} unit={unit} value={values[key]} onChange={(value) => setValue(key,value)} />)}</div></div><div className="form-section"><h3>排放、回收与运行</h3><div className="input-grid">{inputs.slice(10).map(([key,label,unit]) => <NumberField key={key} label={label} unit={unit} value={values[key]} onChange={(value) => setValue(key,value)} />)}</div></div></div>
      ...
      <section className="panel suggestion-panel"><div className="panel-head"><div><h2>优先改进建议</h2><p>按当前输入动态生成</p></div></div><ol>{result.suggestions.map((suggestion,index) => <li key={suggestion}><span>{index + 1}</span><p>{suggestion}</p></li>)}</ol></section></div></section>
  </div>;
}
...
function SimulationModule() {
  ...
  const [points, setPoints] = useState<SimPoint[]>(() => runSimulation(params)); const [visible, setVisible] = useState(1); const [running, setRunning] = useState(false); const [comparison, setComparison] = useState<{ overshoot: number; steadyError: number } | null>(null);
  useEffect(() => { if (!running) return; const timer = window.setInterval(() => setVisible((value) => { if (value >= points.length) { setRunning(false); return value; } return Math.min(points.length, value + Math.max(1, Math.floor(points.length / 180))); }), 40); return () => window.clearInterval(timer); }, [points.length, running]);
  const setParam = (key: keyof SimParams, value: string) => { setParams({ ...params, [key]: Number(value) || 0 }); setRunning(false); };
  const start = () => { const next = runSimulation(params); setPoints(next); if (visible >= next.length) setVisible(1); setRunning(true); };
  const shown = points.slice(0, visible); const current = shown.at(-1) || points[0];
  const metrics = useMemo(() => {
    const maxTemp = Math.max(...points.map((point) => point.temp)); const overshoot = Math.max(0, maxTemp - params.target); const steadyError = Math.abs(points.at(-1)?.error || 0);
    const band = Math.max(params.target * 0.02, 1); let settleTime = params.duration;
  ...
  return <div className="page simulation-page">
    <PageHeader eyebrow="REACTOR SIMULATION" title="反应釜温度智能控制模拟" description="基于能量平衡和离散 PID 算法，观察升温、超调、稳态误差与冷却响应。" actions={<div className="simulation-actions"><button className="button button-outline" onClick={exportCsv}>导出数据</button><button className="button button-primary" onClick={start}>{running ? "重新仿真" : "开始仿真"}</button></div>} />
    <section className="simulation-layout"><aside className="panel parameter-panel"><div className="panel-head"><div><h2>模型参数</h2><p>修改参数后重新开始仿真</p></div></div><div className="param-scroll"><div className="param-group"><h3>过程与设备</h3>{paramFields.map(([key,label,unit]) => <NumberField key={key} label={label} unit={unit} value={params[key]} onChange={(value) => setParam(key,value)} />)}</div><div className="param-group pid-group"><h3>PID 参数</h3><NumberField label="比例系数 Kp" unit="" value={params.kp} onChange={(value) => setParam("kp",value)} /><NumberField label="积分系数 Ki" unit="" value={params.ki} onChange={(value) => setParam("ki",value)} /><NumberField label="微分系数 Kd" unit="" value={params.kd} onChange={(value) => setParam("kd",value)} /></div></div></aside>
      <div className="simulation-main"><section className="sim-kpis"><div><small>实际温度</small><strong>{current.temp.toFixed(1)}<em>℃</em></strong><span className={current.temp >= params.alarm ? "red" : "green"}>{current.temp >= params.alarm ? "高温报警" : "温度正常"}</span></div><div><small>温度偏差</small><strong>{current.error.toFixed(2)}<em>℃</em></strong><span>设定 {params.target}℃</span></div><div><small>加热功率</small><strong>{current.heat.toFixed(0)}<em>%</em></strong><span>最大 {params.maxHeat} kW</span></div><div><small>冷却阀开度</small><strong>{current.cool.toFixed(0)}<em>%</em></strong><span>PID {current.output.toFixed(1)}%</span></div></section>
        <section className="panel chart-panel"><div className="panel-head"><div><h2>温度响应曲线</h2><p>目标温度与实际温度随时间变化</p></div><div className="chart-legend"><span className="target">设定温度</span><span className="actual">实际温度</span><span className="alarm">报警线</span></div></div><LineChart points={shown} alarm={params.alarm} duration={params.duration} /><div className="chart-controls"><button onClick={start}>▶ 开始</button><button onClick={() => setRunning(false)}>Ⅱ 暂停</button><button onClick={() => { setRunning(false); setVisible(1); }}>↺ 重置</button><span>t = {current.time.toFixed(0)} / {params.duration}s</span></div></section>
...
function LineChart({ points, alarm, duration }: { points: SimPoint[]; alarm: number; duration: number }) {
  ...
  useEffect(() => {
    ...
    const draw = () => { const width = parent.clientWidth; const height = 330; const dpr = window.devicePixelRatio || 1; canvas.width = width * dpr; canvas.height = height * dpr; canvas.style.width = `${width}px`; canvas.style.height = `${height}px`; const ctx = canvas.getContext("2d"); if (!ctx) return; ctx.scale(dpr,dpr); ctx.clearRect(0,0,width,height); const pad = {l:48,r:18,t:18,b:35}; const plotW=width-pad.l-pad.r, plotH=height-pad.t-pad.b; const maxY=Math.max(alarm+8,...points.map((p)=>p.temp),...points.map((p)=>p.target)); const minY=Math.min(0,...points.map((p)=>p.temp))-2; const x=(t:number)=>pad.l+t/Math.max(duration,1)*plotW; const y=(v:number)=>pad.t+(maxY-v)/(maxY-minY)*plotH;
